...
│   ├── <host>-services.html        サービス全件（起動設定別の折りたたみ）
...
**この文書は「どう設定されているか」を記録するもので、「今どうなっているか」は対象外とする。**

| 掲載する（定義） | 掲載しない（実行時の状態・時刻） |
|---|---|
| サービスの `start_type` / `start_name` / `path_name` | サービスの `status` |
| `os.os_name` / `cpu_model` / `total_memory_gb` / `timezone` | `os.last_boot` / 空きメモリ |
| ミドルウェアのバージョン・Listen ポート | ミドルウェアの `state` |
| filelist の `type` / `size` / `owner` | filelist の `mtime` |
| `remote_access` の RDP 有効設定・ポート・NLA・SSH の起動設定 | 各サービスの稼働状態 |

`meta.collected_at`（収集日時）と生成日時も掲載しない。

この方針には副次的だが重要な効果がある。**同じ入力からは毎回バイト単位で同じ HTML が出る**ため、
2 世代の出力を diff すれば、タイムスタンプのノイズに埋もれずに**構成変更だけ**を抽出できる。
定義書を Git 管理する運用と相性がよい。

稼働状態の確認は本ツールの役割ではなく、`perf-monitor` / `port-inventory` / `cert-check` など
別のツールが担う。
...
| 手段 | 適用先 |
|---|---|
| 別ページへの分離 | サービス全件（`-services.html`）。詳細ページには総数と自動起動の件数だけを残す |
| 折りたたみ（`<details>`） | サービスの起動設定別一覧、ファイル一覧のディレクトリ階層 |
| ヘッダ固定 + 上部へ戻る | 全ページ。`position: sticky` でナビを常時表示し、長いページ末尾に「▲ ページ上部へ」を置く |
...
サービス全件ページの先頭に起動設定ごとの件数を出し、以下は起動設定ごとに折りたたむ。
稼働状態（`status`）は実行時の情報なので列にも含めない。

起動設定の語彙は OS で違う（Windows は `automatic`、Linux は `enabled`）ため、
「自動起動かどうか」の判定はどちらの語彙も受け付ける。
