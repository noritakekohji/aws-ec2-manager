...
- **PowerShell 5.1 互換必須。** `??` / `?:` / `?.` / `-Encoding utf8NoBOM` は使用禁止
...
`tests/fixtures/env-doc/input/WEB01.snapshot.json`:

```json
{
  "meta": { "hostname": "WEB01", "os_type": "windows", "collected_at": "2026-07-20T10:00:00+09:00", "categories": ["os", "network", "services", "remote_access", "middleware"] },
  "os": { "hostname": "WEB01", "os_name": "Windows Server 2022", "os_version": "10.0.20348", "architecture": "64-bit", "timezone": "Tokyo Standard Time", "cpu_model": "Sample CPU", "cpu_cores": 4, "total_memory_gb": 16.0, "last_boot": "2026-07-19 03:00:00", "hardware": { "virtualization": "virtual" } },
  "network": { "ip_addresses": [{ "interface": "Ethernet", "address": "10.0.1.11" }], "dns_servers": ["10.0.0.2"], "proxy": { "enabled": false }, "ntp": { "servers": ["ntp.example.internal"] } },
  "services": [{ "name": "W3SVC", "status": "Running", "start_mode": "Auto", "service_type": "Own Process", "start_name": "LocalSystem", "path_name": "C:\\Windows\\system32\\svchost.exe -k iissvcs", "description": "サンプル Web サービス" }],
  "remote_access": {
    "rdp": { "enabled": true, "nla_enabled": true, "port_number": "3389", "security_layer": "2", "min_encryption_level": "3" },
    "remote_assistance": { "registry_config": { "fAllowToGetHelp": "0" } },
    "services": [{ "name": "TermService", "display_name": "Remote Desktop Services", "status": "running", "start_type": "manual", "start_name": "NT Authority\\NetworkService", "path_name": "C:\\Windows\\System32\\svchost.exe" }],
    "ssh": { "config_files": {} },
    "firewall_rules": [{ "name": "RemoteDesktop-UserMode-In-TCP", "display_name": "リモート デスクトップ", "direction": "Inbound", "action": "Allow", "profile": "Domain" }]
  },
  "middleware": { "tomcat": [{ "name": "tomcat-1", "catalina_base": "C:\\tomcat", "version": "9.0.85", "java_version": "17.0.9", "state": "running", "connector_ports": [8080], "config_files": {} }] }
}
```

`tests/fixtures/env-doc/input/WEB02.snapshot.json` — `WEB01` と同一構造で、**Tomcat のバージョンだけ `9.0.80` に変える**（不一致ハイライトの検証に使う）。

```json
{
  "meta": { "hostname": "WEB02", "os_type": "windows", "collected_at": "2026-07-20T10:05:00+09:00", "categories": ["os", "network", "services", "remote_access", "middleware"] },
  "os": { "hostname": "WEB02", "os_name": "Windows Server 2022", "os_version": "10.0.20348", "architecture": "64-bit", "timezone": "Tokyo Standard Time", "cpu_model": "Sample CPU", "cpu_cores": 4, "total_memory_gb": 16.0, "last_boot": "2026-07-19 03:05:00", "hardware": { "virtualization": "virtual" } },
  "network": { "ip_addresses": [{ "interface": "Ethernet", "address": "10.0.1.12" }], "dns_servers": ["10.0.0.2"], "proxy": { "enabled": false }, "ntp": { "servers": ["ntp.example.internal"] } },
  "services": [{ "name": "W3SVC", "status": "Running", "start_mode": "Auto", "service_type": "Own Process", "start_name": "LocalSystem", "path_name": "C:\\Windows\\system32\\svchost.exe -k iissvcs", "description": "サンプル Web サービス" }],
  "remote_access": {
    "rdp": { "enabled": true, "nla_enabled": false, "port_number": "3389", "security_layer": "2", "min_encryption_level": "3" },
    "remote_assistance": { "registry_config": { "fAllowToGetHelp": "0" } },
    "services": [{ "name": "TermService", "display_name": "Remote Desktop Services", "status": "running", "start_type": "manual", "start_name": "NT Authority\\NetworkService", "path_name": "C:\\Windows\\System32\\svchost.exe" }],
    "ssh": { "config_files": {} },
    "firewall_rules": [{ "name": "RemoteDesktop-UserMode-In-TCP", "display_name": "リモート デスクトップ", "direction": "Inbound", "action": "Allow", "profile": "Domain" }]
  },
  "middleware": { "tomcat": [{ "name": "tomcat-1", "catalina_base": "C:\\tomcat", "version": "9.0.80", "java_version": "17.0.9", "state": "running", "connector_ports": [8080], "config_files": {} }] }
}
```

`tests/fixtures/env-doc/input/db01.snapshot.json` — **Linux。`middleware` カテゴリを収集していない**（未収集表示の検証に使う）。

```json
{
  "meta": { "hostname": "db01", "os_type": "linux", "collected_at": "2026-07-20T10:10:00+09:00", "categories": ["os", "network", "services", "remote_access"] },
  "os": { "hostname": "db01", "os_name": "Red Hat Enterprise Linux", "os_version": "9.4", "architecture": "x86_64", "timezone": "Asia/Tokyo", "cpu_model": "Sample CPU", "cpu_cores": 8, "total_memory_gb": 32.0, "last_boot": "2026-07-18 22:00:00", "hardware": { "virtualization": "virtual" } },
  "network": { "ip_addresses": [{ "interface": "eth0", "address": "10.0.2.21" }], "dns_servers": ["10.0.0.2"], "proxy": { "enabled": false }, "ntp": { "servers": ["ntp.example.internal"] } },
  "services": [{ "name": "postgresql", "status": "active", "start_mode": "enabled", "fragment_path": "/usr/lib/systemd/system/postgresql.service", "exec_start": "/usr/bin/postmaster -D /var/lib/pgsql/data", "user": "postgres", "group": "postgres", "unit_file_state": "enabled" }],
  "remote_access": {
    "ssh": {
      "services": [{ "name": "sshd.service", "status": "active", "start_type": "enabled" }],
      "config_files": {
        "/etc/ssh/sshd_config": { "content": "Port 22\nPermitRootLogin no\n", "masked": false, "size_bytes": 30, "sha256": "1111111111111111111111111111111111111111111111111111111111111111", "readable": true, "reason": "" }
      }
    },
...
```json
{
  "meta": { "tool": "aws_instance_audit", "hostname": "WEB01", "collected_at": "2026-07-20T10:00:00+09:00", "region": "ap-northeast-1", "instance_id": "i-0aaa1111bbbb2222c" },
  "instance": { "instance_id": "i-0aaa1111bbbb2222c", "instance_type": "t3.medium", "ami_id": "ami-0example", "availability_zone": "ap-northeast-1a", "private_ip": "10.0.1.11", "public_ip": "", "vpc_id": "vpc-0example", "subnet_id": "subnet-0aexample", "tags": { "Name": "WEB01", "Env": "prod" } },
  "iam": { "role_name": "web-instance-role", "role_arn": "arn:aws:iam::000000000000:role/web-instance-role", "attached_policies": [{ "name": "SampleReadOnly", "arn": "arn:aws:iam::aws:policy/SampleReadOnly" }], "inline_policies": ["sample-inline"] },
  "security_groups": [{ "group_id": "sg-0example", "group_name": "web-sg", "description": "web tier", "ingress": [{ "protocol": "tcp", "from_port": 443, "to_port": 443, "cidrs": ["10.0.0.0/16"], "sg_refs": [] }], "egress": [{ "protocol": "all", "from_port": null, "to_port": null, "cidrs": ["0.0.0.0/0"], "sg_refs": [] }] }],
  "network": { "vpc": { "vpc_id": "vpc-0example", "cidr": "10.0.0.0/16", "is_default": false }, "subnet": { "subnet_id": "subnet-0aexample", "cidr": "10.0.1.0/24", "az": "ap-northeast-1a" }, "enis": [{ "eni_id": "eni-0example", "private_ip": "10.0.1.11", "groups": ["sg-0example"] }], "route_tables": [{ "route_table_id": "rtb-0example", "routes": [{ "dest": "0.0.0.0/0", "target": "nat-0example" }] }] }
}
```
...
function Write-EnvDocIndexPage {
    ...
    foreach ($s in $Model.Servers) {
        ...
        if ($s.HasSnapshot) {
            $ips = @(Get-JsonValue -Object $s.Snapshot -Path 'network.ip_addresses' -Default @())
        ...
        foreach ($svc in @(Get-JsonValue -Object $snap -Path 'services' -Default @())) {
            [void]$svcRows.Add(@{
                Cells = @(
                    ...
                    (ConvertTo-HtmlText -Text ([string]$svc.start_mode))
                )
                Mismatch = $false
            })
        }
    ...
    $rows = @(
        @{ Label = 'IP アドレス'; Getter = {
            param($s) Get-EnvDocCategoryValue -Server $s -Category 'network' -Getter {
                param($x) (@(Get-JsonValue -Object $x.Snapshot -Path 'network.ip_addresses' -Default @() |
                    ForEach-Object { '{0}: {1}' -f $_.interface, $_.address }) -join "`n")
            }
        }}
        @{ Label = 'DNS サーバ'; Getter = {
            param($s) Get-EnvDocCategoryValue -Server $s -Category 'network' -Getter {
                param($x) (@(Get-JsonValue -Object $x.Snapshot -Path 'network.dns_servers' -Default @()) -join ', ')
            }
        }}
        @{ Label = 'NTP サーバ'; Getter = {
            param($s) Get-EnvDocCategoryValue -Server $s -Category 'network' -Getter {
                param($x) (@(Get-JsonValue -Object $x.Snapshot -Path 'network.ntp.servers' -Default @()) -join ', ')
            }
        }}
        @{ Label = 'プロキシ'; Getter = {
            param($s) Get-EnvDocCategoryValue -Server $s -Category 'network' -Getter {
                param($x)
                $en = Get-JsonValue -Object $x.Snapshot -Path 'network.proxy.enabled' -Default $false
                if ($en) { [string](Get-JsonValue -Object $x.Snapshot -Path 'network.proxy.server' -Default '有効') } else { '無効' }
            }
        }}
        @{ Label = 'ファイアウォール'; Getter = {
            param($s) Get-EnvDocCategoryValue -Server $s -Category 'security' -Getter {
                param($x) [string](Get-JsonValue -Object $x.Snapshot -Path 'security.firewall.state' -Default '-')
            }
        }}
...
function Get-EnvDocAccessControl {
    ...
    if ($OsType -eq 'windows') {
        return ('UAC: {0}' -f [string](Get-JsonValue -Object $Server.Snapshot -Path 'security.uac.enabled' -Default '-'))
    }
    return ('AppArmor: {0}' -f [string](Get-JsonValue -Object $Server.Snapshot -Path 'security.apparmor.state' -Default '-'))
}
...
function Get-EnvDocTuningSummary {
    ...
    if ($OsType -eq 'windows') {
        $plan = [string](Get-JsonValue -Object $Server.Snapshot -Path 'tuning.power_plan' -Default '-')
        $page = [string](Get-JsonValue -Object $Server.Snapshot -Path 'tuning.pagefile.size_mb' -Default '-')
        return ("電源プラン: {0}`nページファイル: {1}" -f $plan, $page)
    }
    $thp = [string](Get-JsonValue -Object $Server.Snapshot -Path 'tuning.thp_enabled'  -Default '-')
    $gov = [string](Get-JsonValue -Object $Server.Snapshot -Path 'tuning.cpu_governor' -Default '-')
    return ("THP: {0}`nCPU ガバナー: {1}" -f $thp, $gov)
}
...
function Write-EnvDocOsBaselinePage {
    ...
    foreach ($osType in $osTypes) {
        ...
        $osRows = @(
            ...
            @{ Label = 'スケジュール'; Getter = { param($s) Get-EnvDocCategoryValue -Server $s -Category 'scheduled' -Getter { param($x) ('{0} 件' -f @(Get-JsonValue -Object $x.Snapshot -Path 'scheduled' -Default @()).Count) } } }
            @{ Label = 'ファイアウォール'; Getter = { param($s) Get-EnvDocCategoryValue -Server $s -Category 'security' -Getter { param($x) [string](Get-JsonValue -Object $x.Snapshot -Path 'security.firewall.state' -Default '-') } } }
...
`tests/fixtures/env-doc/input/WEB01.snapshot.json` の `meta.categories` に `"packages"` と `"environment"` を追加し、次のキーを追加する。

```json
  "packages": [
    { "name": "Sample App", "version": "1.2.3", "publisher": "Example Inc." },
    { "name": "Sample Runtime", "version": "8.0.1", "publisher": "Example Inc." }
  ],
  "environment": { "SAMPLE_HOME": "C:\\Sample", "PATH": "C:\\Windows;C:\\Sample\\bin" }
```

`middleware.tomcat[0]` の `config_files`（Task 2 では空マップ）を埋める。
**配列ではなく、絶対パスをキーにしたマップ**である点に注意する。

```json
      "config_files": {
        "C:\\tomcat\\conf\\server.xml": {
          "content": "<Server port=\"8005\"><!-- sample --></Server>",
          "masked": false, "size_bytes": 48,
...
さらに `services` 由来と読み取り不可ケースも検証するため、`services[0]` に追記する。

```json
      "config_files": {
        "C:\\ProgramData\\ssh\\sshd_config": {
          "content": "", "masked": false, "size_bytes": 0, "sha256": "",
          "readable": false, "reason": "permission_denied"
        }
      }
```

`tests/fixtures/env-doc/system.yaml` の `WEB01` に opt-in を追加する。
...
```powershell
    Context '全件ページと opt-in' {
        ...
        It 'services 由来の設定ファイルも掲載する' {
            ...
            $html | Should -BeLike '*service W3SVC*'
        }
    ...
    foreach ($p in $pkgs) {
        [void]$rows.Add(@{
            Cells = @(
                ...
                (ConvertTo-HtmlText -Text ([string](Get-JsonValue -Object $p -Path 'publisher' -Default '')))
            )
            Mismatch = $false
        })
    }
    ...
    if ($Server.ShowEnvironment -and (Test-EnvDocCategory -Server $Server -Category 'environment')) {
        ...
        if ($null -ne $envObj) {
            foreach ($prop in $envObj.PSObject.Properties) {
                [void]$envRows.Add(@{
                    Cells = @((ConvertTo-HtmlText -Text $prop.Name), (ConvertTo-HtmlText -Text ([string]$prop.Value)))
                    Mismatch = $false
                })
            }
        }
        $t = New-HtmlTable -Headers @('変数名', '値') -Rows $envRows.ToArray()
