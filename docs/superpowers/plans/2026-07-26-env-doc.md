...
- **フィクスチャは収集ツールの実装から導出したもので、テストより優先する。**
  テストの期待値がフィクスチャと食い違ったら、**必ずテスト側を直す**。
  フィクスチャをテストに合わせて書き換えてはならない（実データと乖離し、
  「テストは通るが実環境では動かない」状態に戻るため）。
  食い違いを見つけたらコントローラに報告すること
...
Describe 'EnvDoc 統合' {
    ...
    It '相対リンクに切れがない' {
        ...
        foreach ($f in $files) {
            ...
            # href だけでなく img src も対象にする(構成図の相対パス切れを拾うため)
...
- Produces:
  ...
  - `New-EnvDocCrossTable -Model <hashtable> -Servers <array> -Rows <array>` → 横断表 HTML
    ...
    - `NoCompare = $true` を付けた行は**不一致判定の対象外**になる。
      最終起動時刻・空きメモリのような揮発値に付けること
...
- Produces:
  ...
  - `Add-EnvDocConfigFileBlock -Body -ConfigMap -Owner -ShowConfigs` → 追加した件数を返す。
    `-ShowConfigs` が `$false` なら何も出さず 0 を返す（呼び出し元 1 箇所への依存を避けるための二重ガード）
...
```powershell
    Context '全件ページと opt-in' {
        ...
        It '読み取り不可のファイルは本文を出さず理由を出す' {
            ...
            # 本文ブロックは readable な 2 件(server.xml / catalina.properties)だけ。
            # 読み取り不可の 2 件(services と remote_access の sshd_config)は本文を出さない。
