...
├── servers/<host>-services.html   サービス全件（起動設定別の折りたたみ）
...
**この定義書は「どう設定されているか」を書くもので、「今どうなっているか」は載せません。**

| 載せる（定義） | 載せない（実行時の状態・時刻） |
|---|---|
| サービスの起動設定・実行ユーザー・実行ファイル | サービスの稼働状態（running / stopped） |
| OS・CPU・搭載メモリ・タイムゾーン | 最終起動時刻・空きメモリ |
| ミドルウェアのバージョン・Listen ポート | ミドルウェアの稼働状態 |
| ファイルの種別・サイズ・オーナー | ファイルの更新日時 |
| RDP/SSH の有効設定・ポート・NLA | RDP/SSH サービスが今動いているか |

収集日時・生成日時も載せません。その結果、**同じ入力からは毎回まったく同じ HTML が出ます**。
2 世代を diff すれば、タイムスタンプのノイズに埋もれず**本当の構成変更だけ**が浮かびます。

稼働状態の確認は `perf-monitor` や `port-inventory` など別のツールの役割です。
...
| 情報 | 見せ方 |
|---|---|
| サービス | サーバ詳細には **総数・自動起動の件数** だけを出し、全件は `-services.html` へ |
| ファイル一覧 | `rel_path` を分解して **ディレクトリツリー**に組み直し、各階層を折りたたむ |
| パッケージ / 設定ファイル | 従来どおり別ページ |

サービス全件ページの先頭には **起動設定ごとの件数**を出し、以下は起動設定ごとに折りたたみます。

折りたたみは `<details>` だけで実現しており JavaScript は使いません。
ページ内の目次（`#` アンカー）から各セクションへ直接飛べます。
ヘッダは画面上部に固定されるので、どこまでスクロールしてもナビに戻れます。
長いページの末尾には「▲ ページ上部へ」を置いています。
